...
FASTAmodule=function(div){
    ...
    dt.body.map(function(x){
        nnn +=x.length;
        var m = x.match(/TACGTGG/gi);
        if(!!m){
            n+=m.length;
            nn++;
        }  
    })
    $('#divHeadBody',div)[0].innerHTML='<span style="color:blue">CpG count (/TACGTGG/gi): '+n+' hits in '+nn+' sequences, accounting for '+n*600/nnn+'% of the sequences length.</span>';
}
